const fs = require('fs');
const pageFile = 'app/industries/page.js';
        ...
        {filteredIndustries.length > 0 ? (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {filteredIndustries.map((industry, i) => {
              ...
              // Use the index from the original INDUSTRIES array for consistent colors even when filtering
              const originalIndex = INDUSTRIES.indexOf(industry);
              const colorClass = colors[originalIndex % colors.length];

              return (
                <Link href={\`/industry/\${slugify(industry)}\`} key={industry}>
